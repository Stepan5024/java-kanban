...
class TaskTest {
    ...
    void checkTwoDuplicatesTaskById(){
        ...
        assertEquals(task, task2, String.format("task1 != task2 by id. Expected that their id should be %d", task.getId()));
    }

    @Test
    void checkTwoDuplicatesInheritorTaskById(){
        // 2. проверьте, что наследники класса Task равны друг другу, если равен их id;
        Subtask subtask1 = new Subtask("Subtask Title", "Subtask description", TaskStatus.NEW, 1, 1);
        Subtask subtask2 = new Subtask("Subtask Title", "Subtask description", TaskStatus.NEW, 1, 1);
        assertEquals(subtask1, subtask2, String.format("subtask1 != subtask2 by id. Expected that their id should be %d", task.getId()));
    }
}